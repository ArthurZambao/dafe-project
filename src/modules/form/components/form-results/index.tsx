...
import { deleteForm, getFormResults } from '@/libs/services/forms/formService';
...
export function FormResults({ formId }: FormResultsProps) {
  ...
  const handleDeleteForm = async (id: string) => {
    try {
      ...
      console.error(err);
      setError('Erro ao deletar o formulário.');
    }
  };
  ...
  return (
    <>
      <ConfirmModal
        ...
        message="Tem certeza que deseja deletar este formulário? Esta ação é irreversível."
        confirmText="Deletar"
        cancelText="Cancelar"
        onConfirm={() => {
      ...
      <div className="p-6 sm:p-10 max-w-7xl mx-auto min-h-screen animate-fade-in">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8 border-l-8 border-azure-primary">
          ...
          <p className="text-slate-gray text-lg">
            Total de Submissões:{' '}
            <span className="font-bold text-azure-primary text-2xl ml-2">
              {data.totalRespostas}
            </span>
          </p>
        </div>
